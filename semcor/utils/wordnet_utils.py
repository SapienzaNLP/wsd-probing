...
def lemma_object_from_string(lemma_string: str) -> wn.lemma:
    return wn.lemma(lemma_string[7:-2])


# @AUTHOR: @simone.teglia
def get_sense_position(lemma_string: str) -> int:
    lemma = lemma_object_from_string(lemma_string)
    synsets_list = wn.synsets(lemma.name(), lemma.synset().pos())
    return synsets_list.index(lemma.synset())
